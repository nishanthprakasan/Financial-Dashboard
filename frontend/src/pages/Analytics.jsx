  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const [monthlyRes, categoryRes, paymentRes] = await Promise.all([
        fetch("http://localhost:8000/api/analytics/monthly", opts),
        fetch("http://localhost:8000/api/analytics/categories", opts),
        fetch("http://localhost:8000/api/analytics/payment-methods", opts),
      ]);
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <p className="text-2xl font-bold text-slate-900">
                ${monthlyData.reduce((s, d) => s + d.expenses, 0).toLocaleString()}
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent className="p-6">
                ...
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(value, name, props) =>
                      [`$${value.toLocaleString()} (${props.payload.count} transactions)`, name]
                    }
                    contentStyle={{ backgroundColor: "#fff", border: "1px solid #e5e7eb" }}
                  />
                </PieChart>
              ...
              <div className="mt-4 space-y-2">
                {paymentData.map((item, index) => (
                  <div key={item.paymentMethod} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      <span
                        className="w-3 h-3 rounded-full"
                        style={{ backgroundColor: PIE_COLORS[index % PIE_COLORS.length] }}
                      />
                      <span className="text-slate-700 capitalize">{item.paymentMethod}</span>
                    </div>
                    <div className="flex gap-4 text-slate-500">
